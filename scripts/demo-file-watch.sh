...
# Purpose: Append visible changes to a file so Dragonfly's auto-upload prompt can be triggered.
# Run: bash scripts/demo-file-watch.sh /path/to/file [iterations] [delay_seconds]
# Docs: docs-site/docs/guide/file-transfer.md
# Settings: First open a remote file from Dragonfly so the local temp copy is being watched.
...
if [[ ! -f "$TARGET_FILE" ]]; then
  printf '# Dragonfly file watch demo\n' > "$TARGET_FILE"
fi
...
printf 'File watch demo finished. If Dragonfly opened this file, you should have seen upload prompts.\n'
